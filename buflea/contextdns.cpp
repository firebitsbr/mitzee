...
//    _hdr.replace_option(_hdr._nhost,_hdr._nhost_end,"enjoydecor.com");
    ...
    if(_rock.sendall((const u_int8_t*)_hdr.buf(), _hdr.bytes(), SS_TOUT)!=0)
    {
        return R_KILL;
    }
    _clear_header();
    _pcall=(PFCLL)&CtxDns::_transfer;
    return R_CONTINUE;
}
