...
struct SQFile : public SQStream {
	...
	bool Open(const SQChar *filename ,const SQChar *mode) {
		...
		if( (_handle = sqstd_fopen(filename,mode)) ) {
		    //::fcntl(fileno(pipe), F_SETFD, FD_CLOEXEC);//mco
			_owns = true;
			return true;
		}
		return false;
...
SQRESULT sqstd_dofile(HSQUIRRELVM v,const SQChar *filename,SQBool retval,SQBool printerror)
{
    ...
    ::sprintf(fname,"%s%s", __path, filename);
	if(SQ_SUCCEEDED(sqstd_loadfile(v,fname,printerror))) {
		sq_push(v,-2);
...
SQInteger _g_io_chdir(HSQUIRRELVM v)
{
	const SQChar *dirname;
	SQBool printerror = SQFalse;
	sq_getstring(v,2,&dirname);
    strcpy(__path, dirname);
...
static SQRegFunction iolib_funcs[]={
	...
	_DECL_GLOBALIO_FUNC(chdir,-2,_SC(".sb")),
...
SQRESULT sqstd_register_iolib(HSQUIRRELVM v)
{
	SQInteger top = sq_gettop(v);
	//create delegate
	declare_stream(v,_SC("file"),(SQUserPointer)SQSTD_FILE_TYPE_TAG,_SC("std_file"),
                    _file_methods,iolib_funcs);
