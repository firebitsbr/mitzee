...
inline u_int64_t extractRanges(kchar* pz, u_int64_t b, u_int64_t& s, u_int64_t& e)
{
    ...
    if(*(pd+1))
    {
        sscanf(pz, "bytes=%llu-%llu", (long long unsigned int*)&s, (long long unsigned int*)&e);
        return e-s+1;
    }
    sscanf(pz, "bytes=%llu-", (long long unsigned int*)&s);
    return b-s;
}

#define     SECS(k_)                (k_*100)
#define APP_ERRBASE                 0x8000
#define APP_LASTERROR               APP_ERRBASE+21
struct Mex
{
private:
    int     _code;
